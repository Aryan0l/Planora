...
const isFollowingPlan = async (userId, planId) => {
    const result = await pool_1.default.query('SELECT id FROM followers WHERE user_id = $1 AND plan_id = $2', [userId, planId]);
    return result.rows.length > 0;
};
exports.isFollowingPlan = isFollowingPlan;
const addFollower = async (userId, planId) => {
    await pool_1.default.query(`INSERT INTO followers (user_id, plan_id)
     VALUES ($1, $2)
...
const removeFollower = async (userId, planId) => {
    await pool_1.default.query('DELETE FROM followers WHERE user_id = $1 AND plan_id = $2', [userId, planId]);
};
exports.removeFollower = removeFollower;
const getFollowerCount = async (planId) => {
    const result = await pool_1.default.query('SELECT COUNT(*) FROM followers WHERE plan_id = $1', [planId]);
