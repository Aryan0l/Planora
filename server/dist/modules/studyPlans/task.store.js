...
const insertPlanTasks = async (planId, tasks) => {
    ...
    const result = await pool_1.default.query(`INSERT INTO plan_tasks (plan_id, day_number, title, description)
     VALUES ${inserts.join(', ')} RETURNING *`, values);
    return result.rows;
};
exports.insertPlanTasks = insertPlanTasks;
const getPlanTasks = async (planId) => {
    const result = await pool_1.default.query(`SELECT id, day_number AS day, title, description
     FROM plan_tasks
     WHERE plan_id = $1
...
const deletePlanTasks = async (planId) => {
    await pool_1.default.query('DELETE FROM plan_tasks WHERE plan_id = $1', [planId]);
};
exports.deletePlanTasks = deletePlanTasks;
const countPlanTasks = async (planId) => {
    const result = await pool_1.default.query('SELECT COUNT(*) FROM plan_tasks WHERE plan_id = $1', [planId]);
