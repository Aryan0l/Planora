...
const getCompletedTaskIds = async (userId, planId) => {
    if (await hasCompletedTaskIdsArray()) {
        const result = await db_1.default.query(`SELECT completed_task_ids
       FROM progress
...
const insertProgress = async (userId, planId, taskIds) => {
    ...
    if (await hasCompletedTaskIdsArray()) {
        await db_1.default.query(`INSERT INTO progress (user_id, plan_id, completed_task_ids, updated_at)
...
const countCompletedTasks = async (userId, planId) => {
    if (await hasCompletedTaskIdsArray()) {
        const result = await db_1.default.query(`SELECT COALESCE(array_length(completed_task_ids, 1), 0) AS count
       FROM progress
       WHERE user_id = $1 AND plan_id = $2`, [userId, planId]);
